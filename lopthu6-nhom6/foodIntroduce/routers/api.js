module.exports = (express, connection) => {
    ...
    // Simple MySQL Test
    router.get('/getallmonan', (req, res) => {
        var test;

        connection.query('SELECT * FROM monan order by SoLuotXem DESC LIMIT 8', (err, rows, fields) => {
            if (err) throw err;
    ...
    router.get('/getallcomment', (req, res) => {
        ...
        connection.query('SELECT * FROM comment order by updated_at DESC', (err, rows, fields) => {
            if (err) throw err;
    ...
    router.get('/comment/:id', (req, res) => {
        ...
        connection.query('SELECT * FROM comment where id=?',[req.params.id], (err, rows, fields) => {
            if (err) throw err;
    ...
    router.post('/addComment', (req, res) => {
        ...
        var query = connection.query('Insert into comment (NoiDung, ten_user, id_MonAn, created_at) values(?,?,?,?)',[data.NoiDung, data.ten_user, data.id_MonAn, data.created_at], (err, result) => {
            if (err) {
                console.error(err);
                res.sendStatus(404);
            } else {
                res.status(201);
                res.location('/api/comment/' + result.id);
                res.end();
            }
        });
    });
...
// COLLECTION ROUTES
router.route('/comment/:idmonan')
    //we can use .route to then hook on multiple verbs

    .get((req, res) => {
        var query = connection.query('SELECT * FROM comment WHERE id_MonAn=?', [req.params.idmonan], (err, rows, fields) => {
...
router.route('/monan/:id')
    ...
    .get((req, res) => {
        var query = connection.query('SELECT * FROM monan WHERE id=?', [req.params.id], (err, rows, fields) => {
            if (err) {
                //INVALID
                console.error(err);
                res.sendStatus(404);
    ...
    .put((req, res) => {
        var data = req.body;
        var query = connection.query('UPDATE monan SET ? WHERE id=?', [data, req.params.id], (err, result) => {
            if (err) {
                console.log(err);
                res.sendStatus(404);
            } else {
    ...
    .delete((req, res) => {
        ...
        var query = connection.query('DELETE FROM monan WHERE id=? LIMIT 1', [req.params.id], (err, result) => {
            if (err) {
                console.log(err);
                res.sendStatus(404);
            } else {
